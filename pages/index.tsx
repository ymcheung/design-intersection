...
const Home: NextPage<postsProps> = ({ posts }) => {
  ...
  const featuredPosts = posts.filter(({ tags }) => tags.includes("featured"));
  ...
  return (
    <>
      ...
      >
        <HomeCell
          as="main"
          position={{ "@m992": "featured" }}
          responsive={{ "@m992": "tablet" }}
        >
          <PostList>
            {featuredPosts.map(
              ({ slug, title, cover, description, publishedTime }, index) => (
          ...
          <Link href="https://thecosignstudio.github.io/process" passHref>
            <WebsiteProcessLink>
              ...
              >
                <WebsiteProcessImage
                  src="/website/process.webp"
                  fill={true}
                  alt="專業人士的設計流程 (Design Process for Pros)"
                />
              </WebsiteProcess>
            </WebsiteProcessLink>
          </Link>
          ...
          <TranslatorCard
            responsive={{ "@initial": "mobile", "@m992": "tablet" }}
          >
            <TranslatorAvatar src={ymcheung} width={80} height={80} alt="" />
            <figcaption>
              <Heading position="translator">Yuming Cheung</Heading>
              <Description position="translator">
                網站前端開發 @RE:LAB
              </Description>
              <Link href="https://read.cv/ymcheung" passHref>
                <Description as="a" position="translatorLink">
                  read.cv/ymcheung
                </Description>
              </Link>
            </figcaption>
              ...
              <PostItem key={`all_${index}`}>
                <Link href={`/${slug}`} passHref>
                  <PostLink>
                    ...
                    <Heading as="h3" position="postsAll">
                      {title}
                    </Heading>
                  </PostLink>
                </Link>
