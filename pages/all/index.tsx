...
const All: NextPage<postsProps> = ({ posts }) => {
  ...
  const byYear = () => {
    const allYears = posts.map(({ publishedTime }) =>
      handleTimeToYear(publishedTime)
    );
    const uniqueYears = [...new Set(allYears)];

    const filterByYear = uniqueYears.map((year) => {
      const yearPosts = posts.filter(
        ({ publishedTime }) => handleTimeToYear(publishedTime) === year
      );
                  ...
                  <ListItem key={slug}>
                    <Link href={`/${slug}`} passHref>
                      <PostLink>
                        ...
                        <Heading as="h2" position="postsAll">
                          {title}
                        </Heading>
                      </PostLink>
                    </Link>
